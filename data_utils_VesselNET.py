...
class SegmentationData(data.Dataset):
    ...
    def __init__(self, image_paths_file, transform = None, mode = 'train'):
        ...
        self.num_train = 3381   #60%
        self.num_val = 1127     #20%
        self.num_test = 1127    #20%
    ...
    def get_item_from_index(self, index):
        ...
        target_binary = int(np.sum(target_labels) > 0)
        target_onehot = np.zeros(2)
        target_onehot[target_binary] = 1
        
        target_labels = torch.from_numpy(target_labels.copy())
        target_onehot = torch.from_numpy(target_onehot.copy())
        target_onehot = target_onehot.type(torch.FloatTensor)
        
        return img, [target_labels, target_onehot]
