...
class NerveNET(nn.Module):
    ...
    def __init__(self, input_dim = (1, 128, 128), num_classes=2,
                 weight_scale = True, dropout = 0.25, leak = 0.01,
        ...
        if upsample_unit == 'Upsample':
            upsample_units = []
            upsample_unit = [nn.Upsample(scale_factor = 2) for x in range(4)]
        elif upsample_unit == 'ConvTranspose2d':
            upsample_units = []
            upsample_units[0] = nn.ConvTranspose2d(512,512,kernel_size=2,stride=2,padding=0)
            upsample_units[1] = nn.ConvTranspose2d(256,256,kernel_size=2,stride=2,padding=0)
            upsample_units[2] = nn.ConvTranspose2d(128,128,kernel_size=2,stride=2,padding=0)
            upsample_units[3] = nn.ConvTranspose2d(64, 64,kernel_size=2,stride=2,padding=0)
        else:
            ValueError
    ...
    def forward(self, x):
        ...
        if self.binary_out:
            out['binary'] = self.binary(self.pool(down4)) 
